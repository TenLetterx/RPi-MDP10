...
class TaskOne(RaspberryPi):
    ...
    def recognize_image(self, obstacle_id_with_signal: str) -> None:
        ...
        try:
            # Assuming results looks like {"id": "ArrowLeft"} or {"target_id": "A1"}
            target_id = results.get("id") or results.get("target_id") or str(results)
            formatted_msg = f"TARGET,{obstacle_id},{target_id}"
