...
class TaskOne(RaspberryPi):
    ...
    # Clear all command and path queues
        ...
        while True:
            ...
            try:
                ...
                # --- Obstacle packet ---
                    ...
                    # Remove any existing obstacle at these coordinates
                    obstacles_accum = [obs for obs in obstacles_accum if not (obs["x"] == x and obs["y"] == y)]

                    # Add the new/updated obstacle
                ...
                # --- STOP command ---
                elif android_str.upper() == "STOP":
                    logger.info("Received STOP command from Android - sending emergency stop to STM")
                    
                    # 1. Send emergency stop command 'X\n' to STM
                    try:
                        self.stm_link.send_cmd_raw(b"X\n")
                        logger.info("[STM] Sent emergency stop command 'X'")
                    except Exception as e:
                        logger.error(f"Failed to send STOP to STM: {e}")
                    
                    # 2. Clear all command queues to prevent further movement after reset
                    self.clear_queues()
                                        
                    # 3. Release movement lock if held (robot is now hard-stopped)
                    if self.movement_lock:
                        try:
                            self.movement_lock.release()
                            logger.info("Movement lock released due to emergency stop")
                        except Exception:
                            logger.debug("Movement lock already released")
                    
                    # 4. Pause command execution
                    self.unpause.clear()
                    
                    # 5. Notify Android about the emergency stop state
                    self.android_queue.put(AndroidMessage(Category.STATUS.value, "emergency_stop"))
                    self.android_queue.put(AndroidMessage(Category.INFO.value, "Robot emergency stopped. Reset required on STM."))
    ...
    # [JX 1 oct 12pm, i pasted this whole def recognize_image chunt from chatgpt]
    def recognize_image(self, obstacle_id_with_signal: str) -> None:
        ...
        # ✅ Build Android-friendly message
        try:
            # results looks like {'obstacle_id': '3', 'image_id': '20'}
            obstacle_num = obstacle_id
            target_id = results.get("image_id", "NA")   # default to NA if not present

            formatted_msg = f"TARGET,{obstacle_num},{target_id}"

            # Send clean string only
            self.android_queue.put(AndroidMessage(Category.IMAGE_REC.value, value=formatted_msg))
            logger.info(f"Sent to Android: {formatted_msg}")
        except Exception as e:
            logger.error(f"Failed to format image recognition result: {results} ({e})")
