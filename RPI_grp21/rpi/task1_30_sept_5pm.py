...
class TaskOne(RaspberryPi):
    ...
    def command_follower(self) -> None:
        ...
        while True:
            ...
            if command_str.startswith(("T", "t", "W", "w", "S")):
                # 🚗 Movement / trajectory commands for STM
                try:
            ...
            elif command_str.upper().startswith("SNAP"):
                # 📸 Handle snapshot locally, not via STM
                logger.info(f"[SNAP] Triggered snapshot command: {command_str}")
                # Remove the "SNAP" prefix but keep obstacle/signal info
                obstacle_id_with_signal = command_str.replace("SNAP", "")
                self.rpi_action_queue.put(
                    PiAction(cat=Category.SNAP, value=obstacle_id_with_signal)
                )
                if self.movement_lock and self.movement_lock._semlock._is_mine():
                    try:
                        self.movement_lock.release()
                    except Exception:
                        logger.debug("movement_lock already released (SNAP), skipping.")

            elif command_str == Category.FIN.value:
                # ✅ End of sequence
