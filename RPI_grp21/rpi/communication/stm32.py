...
class STMLink(Link):
    ...
    def send_cmd_raw(self, cmd: str | bytes) -> None:
        if isinstance(cmd, str):
            ...
            logger.error(f"Unsupported command type: {type(cmd)}")
            return

        self.serial_link.write(payload)
        logger.debug(f"[STM] Sent raw: {payload!r}")
